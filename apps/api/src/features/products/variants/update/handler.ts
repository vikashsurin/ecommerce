...
import { type UpdateProductVariantSchema, updateProductVariantSchema } from './schema';

export const updateProductVariantApp = appFactory()
...
async function updateProductVariant(productId: number, variantId: number, data: UpdateProductVariantSchema) {
  const row = await db
