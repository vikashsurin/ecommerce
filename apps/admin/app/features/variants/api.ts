import { apiClient, rpcClient } from "@/lib";
...
export const uploadVariantImages = async (variantId: number, isPrimary: boolean, files: File[]) => {
  ...
  try {
    ...
    const confirmRes = await apiClient.api.variants[":variantId"].images.confirm.$post({
      param: { variantId: String(variantId) },
      json: {
        images: presigned.map((p, i) => ({ key: p.key, sortOrder: i, isPrimary: isPrimary })),
      },
    });
